...
const HoraExtra = () => {
  ...
  // Delete Extra Hour Request
  const eliminarHoraExtra = async (fecha, idEmpleado) => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/horas-extras/${formatDate(fecha)}/${idEmpleado}`);
      setAlertModal({ visible: true, message: response.data.mensaje, type: 'success' });

      obtenerHorasExtras(); // Refresh data after deletion
    } catch (error) {
      const errorMessage = error.response && error.response.status === 400
        ? error.response.data.error
        : 'Error al eliminar la solicitud de horas extras';
      setAlertModal({ visible: true, message: errorMessage, type: 'error' });
