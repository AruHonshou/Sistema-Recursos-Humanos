...
const ConsultarLiquidacion = () => {
  ...
  // Obtener empleado basado en el idUsuario en localStorage
  const obtenerEmpleado = async () => {
    try {
      ...
      if (idusuario) {
        ...
        setEmpleados([response.data]); // Wrap in array to map over in display
  ...
  const liquidacionesFiltradas = liquidaciones.filter((liquidacion) => {
    const fechaLiquidacion = new Date(liquidacion.Fecha_Liquidacion).getTime();
    const inicioFiltro = fechaInicio ? new Date(fechaInicio).getTime() : null;
    const finFiltro = fechaFin ? new Date(fechaFin).getTime() : null;

    const empleadoMatch = idEmpleado === '' || liquidacion.empleados_idEmpleado === parseInt(idEmpleado);
    const fechaMatch = (!inicioFiltro || fechaLiquidacion >= inicioFiltro) && (!finFiltro || fechaLiquidacion <= finFiltro);
  ...
  return (
    <div className="p-6 bg-[#f9f9f9] dark:bg-[#1E1E2F] min-h-screen">
      <h1 className="text-2xl font-bold mb-4 text-black dark:text-white text-center">Consultar Liquidación</h1>

      <div className="flex justify-between mb-4 gap-4">
        <button onClick={generarPDF} className="bg-[#222831] text-[#00ADB5] py-2 px-6 rounded-lg shadow-lg transition duration-200 ease-in-out transform hover:scale-105 hover:bg-[#393E46] focus:outline-none focus:ring-2 focus:ring-[#00ADB5]">
          Exportar a PDF
        </button>
        <button onClick={generarExcel} className="bg-[#222831] text-[#00ADB5] py-2 px-6 rounded-lg shadow-lg transition duration-200 ease-in-out transform hover:scale-105 hover:bg-[#393E46] focus:outline-none focus:ring-2 focus:ring-[#00ADB5]">
          Exportar a Excel
        </button>
      </div>
...
export default ConsultarLiquidacion;
